        ...
        <div class="row justify-content-center">
            <div class="col-12 col-md-10 col-lg-8 col-xl-6">
                ...
                <div class="text-center mb-5">
                    ...
                    <p class="investment-subtitle" style="color: var(--color-text-secondary);">Update your investment
                        details</p>
                </div>
                ...
                <form method="POST" action="{{ route('investments.update', $investment->id) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group input-group-lg">
                            ...
                            <input type="number" class="form-control @error('investment_amount') is-invalid @enderror"
                                ...
                                step="100" value="{{ old('investment_amount', $investment->investment_amount) }}"
                                required>
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select form-control-lg @error('payment_method') is-invalid @enderror"
                            ...
                            <option value="upi_gpay"
                                {{ old('payment_method', $investment->payment_method) == 'upi_gpay' ? 'selected' : '' }}>
                                UPI (GPay/PhonePe/Paytm)</option>
                            <option value="bhim_upi"
                                {{ old('payment_method', $investment->payment_method) == 'bhim_upi' ? 'selected' : '' }}>
                                BHIM UPI</option>
                            <option value="imps"
                                {{ old('payment_method', $investment->payment_method) == 'imps' ? 'selected' : '' }}>
                                IMPS</option>
                            <option value="neft_rtgs"
                                {{ old('payment_method', $investment->payment_method) == 'neft_rtgs' ? 'selected' : '' }}>
                                NEFT/RTGS</option>
                            <option value="usdt_trc20"
                                {{ old('payment_method', $investment->payment_method) == 'usdt_trc20' ? 'selected' : '' }}>
                                USDT TRC20</option>

                            <option value="net_banking"
                                {{ old('payment_method', $investment->payment_method) == 'net_banking' ? 'selected' : '' }}>
                                Net Banking</option>
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text"
                            ...
                            value="{{ old('upi_or_bank', $investment->upi_id_or_bank) }}" required>
                        @error('upi_or_bank')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text"
                            ...
                            value="{{ old('transaction_id', $investment->transaction_id) }}" required>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group input-group-lg">
                            ...
                            <input type="number" class="form-control @error('paid_amount') is-invalid @enderror"
                                ...
                                value="{{ old('paid_amount', $investment->paid_amount) }}" required>
                        </div>
                    ...
                    <!-- Payment Screenshot -->
                    <div class="mb-4">
                        <label for="paymentScreenshot" class="form-label fw-600"
                            style="color: var(--color-text-primary);">Payment Screenshot <span class="text-muted">(Leave
                                empty to keep current)</span></label>

                        @if($investment->payment_proof)
                        <div class="mb-3">
                            <small class="text-muted">Current Screenshot:</small>
                            <div class="mt-2">
                                <img src="{{ Storage::url($investment->payment_proof) }}" alt="Current Payment Proof"
                                    style="max-height: 150px; border-radius: var(--radius-md); border: 1px solid var(--color-border-primary);">
                            </div>
                        </div>
                        @endif

                        <input type="file"
                            class="form-control form-control-lg @error('payment_screenshot') is-invalid @enderror"
                            id="paymentScreenshot" name="payment_screenshot" accept=".jpg,.jpeg,.png">
                        <small class="form-text" style="color: var(--color-text-secondary);">Accepted formats: JPG,
                            JPEG, PNG (Max 5MB)</small>
                    ...
                    <div class="mb-4">
                        <label for="transactionDateTime" class="form-label fw-600"
                            style="color: var(--color-text-primary);">Transaction Date & Time <span
                                class="text-danger">*</span></label>
                        <input type="datetime-local"
                            class="form-control form-control-lg @error('transaction_date_time') is-invalid @enderror"
                            id="transactionDateTime" name="transaction_date_time"
                            value="{{ old('transaction_date_time', $investment->transaction_datetime->format('Y-m-d\TH:i')) }}"
                            required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control form-control-lg @error('notes') is-invalid @enderror" id="notes"
                            ...
                            style="resize: vertical;">{{ old('notes', $investment->transactions()->where('type', 'deposit')->first()?->remark) }}</textarea>
                        @error('notes')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="text-center">
                        <a href="{{ route('investments.show', $investment->id) }}"
                            style="color: var(--color-primary);">Back to Investment Details</a>
                    </div>
