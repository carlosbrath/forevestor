...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-header">
        <div class="logo">
            <img src="{{ asset('/assets/images/favicon.png') }}" width="35" height="35" alt="Forevestor Logo" />
            <div class="logo-text">
                <h4>{{ in_array(auth()->user()->role?->name, ['super-admin', 'admin', 'moderator']) ? 'Admin Panel' : 'Dashboard' }}
                </h4>
                <small
                    class="user-role">{{ ucfirst(str_replace('-', ' ', auth()->user()->role?->name ?? 'Guest')) }}</small>
            </div>
        </div>
    </div>
    ...
    <ul class="nav-menu">
        ...
        {{ request()->routeIs('investments.*') ? 'active' : '' }}">
        ...
        <li class="nav-item">
            <a href="{{ route('super-admin.roles') }}"
                class="nav-link {{ request()->routeIs('super-admin.roles') ? 'active' : '' }}">
        ...
        <li class="nav-item">
            <a href="{{ route('super-admin.permissions') }}"
                class="nav-link {{ request()->routeIs('super-admin.permissions') ? 'active' : '' }}">
